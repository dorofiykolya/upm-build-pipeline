...
namespace Common.BuildPipeline.Builders
{
    public class IOSBuilder : DefaultBuilder
    {
        public override void PreBuild(BuildConfiguration config, ILogger logger)
        {
            ...
            args.SetStaticPropertiesFromFields<PlayerSettings.iOS>(ios, true);
            args.OnExist(BuilderArguments.IOS.IPadLaunchScreenType,
                (key, value) =>
                    PlayerSettings.iOS.SetiPadLaunchScreenType(
                        args.GetValueByEnum<iOSLaunchScreenType>(BuilderArguments.IOS.IPadLaunchScreenType)));
            args.OnExist(BuilderArguments.IOS.IPhoneLaunchScreenType,
                (key, value) =>
                    PlayerSettings.iOS.SetiPadLaunchScreenType(
                        args.GetValueByEnum<iOSLaunchScreenType>(BuilderArguments.IOS.IPhoneLaunchScreenType)));
        }
        ...
        public override string Help
        {
            get
            {
                return "iOS" + Environment.NewLine +
                       BuilderUtils.GetArgumentsDescription(typeof(BuilderArguments.IOS), 2);
            }
        }
